...
def _https_url(value: str, field: str) -> str:
    ...
    _require("@" not in host, f"{field} credentials not allowed")
...
def _origin(value: str) -> str:
    return "https://" + value.split("/", 3)[2].lower()
...
class PatchReviewEngine(gl.Contract):
    ...
    def review_candidate(
        ...
        def evaluate() -> dict:
            ...
            for name in ("advisory", "patch", "tests", "deployment"):
...
"""
        ...
        if decision.get("outcome") == "REMEDIATED":
            ...
            authority.emit(on="finalized").authorize(
                rid,
                sid,
                version,
                commit,
